...
* \textbf{if, else}: testing condition

* \textbf{for}: execute a loop a fixed number of times

* \textbf{while}: execute a loop while a condition is true

* \textbf{repeat}: execute an infinite loop

* \textbf{break}: break the execution of a loop

* \textbf{next}: skip an iteration of a loop

* \textbf{skip}: exit a function
...
\textbf{for loops} take an iterator variable and assign it successive values from a sequence or vector. For loops are most commonly used for iterating over the elements of an object (list, vector, etc.)
```{r}
for (i in 1:10){
  print(i)
}
...
```
Let's try \textbf{Nested for loops}
```{r}
x <- matrix(1:6, 2, 3)
...
```
\textit{Note to self: Finish first the full iteration of the inner loop then after that return to outer loop. The loop ends base on the iteration of the outer loop.}

# Control Structures: while

\textbf{while loops} begin by testing a condition. If it is true, then they execute the loop body. Once the loop body is executed, the condition is tested again, and so forth.
```{r error=TRUE}
count <- 0
while(count < 10) {
  print(count)
...
\textbf{Repeat} initiates an infinite loop; these are not commonly used in statistical applications but they do have their uses. The only way to exit a repeat loop is to call \textbf{break}
```{r}
x0 <- 1
to1 <- 1e-8
...
# Control Structures: next
 \textbf{next} is used skip an iteration of a loop
```{r error = TRUE}
for(i 1:100) {
  if(i <= 20) {
...
```
\textit{Note to self: \textbf{next} is a way to skip an iteration in the loop while \textbf{break} is a way to exit the loop entirely (and execute again).}

# Control Structures: return

\textbf{return} signals that a function should exit and return a given value.
...
* \textbf{lapply}: Loop over a list and evaluate a function on each element

* \textbf{sapply}: Same as lapply but try to simplify the result

* \textbf{apply}: Apply a function over the margins of an array

* \textbf{tapply}: Apply a function over subsets of a vector

* \textbf{mlapply}: Multivariate version of lapply
...
```
\textbf{Anonymous Functions} are functions with no names. lapply and friends make heavy use of anonymous functions.
```{r}
...
The shortcut functions are much daster, but you won't notice unless you're using a large matrix.
...
\textbf{mapply} is a multivariate apply of sorts which applies a function in parallel over a set of arguments.
```{r}
str(mapply)
```
...
noise(5,1,2)
```
What if I set n to be vector and mean to be also vector and fixed my standard deviation to  2. Here, we will use the mapply which produce output that is 1 mean, 2 mean, 3 mean, and so on and so forth ...
```{r}
...
\textbf{split} takes a vector or other objects and splits it into groups determined by a factor or list of factors.
```{r}
str(split)
```
...
* \textbf{message}: A generic notification/diagnostic message produced by message function; execution of the function continues

* \textbf{warning}: An indication that something is wrong but not necessarily fatal; execution of the function continues; generated by the warning function

* \textbf{error}: An indication that a fatal problem has occured; execution stops; produced by the stop function.

* \textbf{condition}: A generic concept for indicating that something unexpected can occur; programmers can create their own conditions.
```{r}
log(-1)
```
```{r}
...
* \textbf{traceback}: prints out the function call stack after an error occurs; does nothing if there's no error.

* \textbf{debug}: flags a function for "debug" mode which allow you to step through execution of a function one line at a time

* \textbf{browser}: suspends the execution of a function wherever it is called and puts the function in debug mode

* \textbf{trace}: allows you to insert debugging code into a function a specific places

* \textbf{recover}: allows you to modify the error behavior so that you can browse the function call stack
